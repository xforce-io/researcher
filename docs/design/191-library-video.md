...
有效台词：`text` trim 后非空，且 `end > start`，时间为秒。无效句丢弃；丢光则按成功空处理。不保留分析历史浏览入口；磁盘上可留历次记录供调试，UI 只暴露当前产物与最新一次失败。
...
**详情**：共同框架（返回 Library、标题、类型、更新时间、中央区、支持动作）。video 中央为播放器 + 台词表；动作仅 Analyze（媒体在且运行时允许时）或 Restore media（媒体缺失）。不展示空的 Essence、批注、topic、Edit。

播放器播已入库文件，须支持 Range。媒体缺失时播放器位置换成缺失说明和恢复入口，台词表仍可阅读、搜索；点句/跳转/分析解释缺失，不假装已定位。

台词表：未分析给出 Analyze；分析中禁用重复提交并显示进行中；成功有词显示列表；成功无语音显示 `No speech detected`；失败在保留旧列表（若有）上方显示错误和重试。搜索为详情内子串，大小写不敏感；无命中显示空结果，不清空底层产物。清空搜索恢复筛选前全部句。跟随滚动动效实现自定，标哪一句必须遵守 §4 当前句规则。
